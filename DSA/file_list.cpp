...
void insertbefore() {
    int n, n1, j, i, found = 0;
    
    if (mylist.c == SIZE) {
        printf("New item can't be added!!!\n");
    } else {
        ...
        for (i = 0; i < mylist.c; i++) {
            if (mylist.items[i] == n1) {  // Found the number to insert before
                found = 1;
                ...
                // Insert new number at `i`
                mylist.items[i] = n;
                mylist.c++;

                printf("\n%d inserted successfully before %d\n", n, n1);
...
void insertafter() {
    int n, n1, i, j, found = 0;

    if (mylist.c == SIZE) {
        printf("New item can't be added to the list.\n");
    } else {
        ...
        for (i = 0; i < mylist.c; i++) {
            if (mylist.items[i] == n1) {  // Found `n1` in the list
                found = 1;
                ...
                // Insert new number after `n1`
                mylist.items[i + 1] = n;
                mylist.c++;

                printf("\n%d successfully added after %d.\n", n, n1);
                return;  // Exit function after inserting
            }
        }

        // If `n1` is not found, show error message
        if (!found) {
